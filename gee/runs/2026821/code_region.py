"""Eight-region Python/geemap observed-treeline extraction.
...
``--export`` creates 0.25-degree analysis cells over the complete GMBA Basic
geometries, groups them into bounded 2-degree export shards, and starts one
30 m, 1 km, and QA30m Asset export per shard. Otsu is derived independently
inside each shard from unchanged integer BIO1 values.
"""
...
def resolved_plan(args: argparse.Namespace) -> Dict[str, object]:
    ...
    return {
        ...
        "expected_shard_count": args.expected_shard_count if not args.region_ids else "resolved online",
        "expected_export_task_count": (
            args.expected_shard_count * product_count if not args.region_ids else "resolved online"
        ),
        ...
        "temperature_transform": {
            ...
            "otsu_scope": "dynamic per export shard on unchanged integer BIO1",
        },
        "assets": {
...
def child_asset_id(
    collection_id: str, region_id: str, tile_x: int, tile_y: int, aspect_mode: str
) -> str:
    child = sanitize_asset_component(
        f"{shard_key(region_id, tile_x, tile_y)}_{aspect_mode.replace('-', '_')}"
    )
...
def resolve_shard_plan(
    ...
    if not args.region_ids and len(plan) != args.expected_shard_count:
        raise ValueError(
...
def planned_export_records(
    ...
    product_specs = [
        ("treeline30m", args.treeline30m_collection, FINE_CRS, FINE_TRANSFORM, "mean"),
        ("treeline1km", args.treeline1km_collection, CLIMATE_CRS, CLIMATE_TRANSFORM, "mean"),
    ]
    if args.export_qa:
        product_specs.append(("qa30m", args.qa30m_collection, FINE_CRS, FINE_TRANSFORM, "mode"))
    ...
    for shard in shard_plan:
        ...
        for product, collection_id, crs, transform, pyramiding_policy in product_specs:
            description = sanitize_asset_component(
                f"{args.task_prefix}_{current_shard_key}_{args.aspect_mode}_{product}"
            )
            records.append(
                {
                    "region_id": region_id,
...
def quarter_degree_screen(processing_region: ee.Geometry, args: argparse.Namespace) -> ee.Image:
    grid_projection = quarter_grid_projection(args)
    climate_projection = ee.Projection(CLIMATE_CRS, CLIMATE_TRANSFORM)
    worldcover_tree = (
        ee.ImageCollection(WORLDCOVER).first().select("Map").eq(10)
        .clip(processing_region).unmask(0)
    )
    tree_at_climate_scale = (
        worldcover_tree.reduceResolution(reducer=ee.Reducer.mean(), maxPixels=16384)
        .reproject(climate_projection).clip(processing_region)
    )
    tree_fraction = (
        tree_at_climate_scale.reduceResolution(reducer=ee.Reducer.mean(), maxPixels=4096)
        .reproject(grid_projection).clip(processing_region)
        .rename("tree_cover_fraction")
    )
    return tree_fraction.lte(args.maximum_tree_cover_fraction).rename("valid_quarter_degree_cell")
...
def build_common(
    ...
    if selected_gmba is None:
        selected_gmba = select_manifest_gmba(args)[0].filterBounds(processing_region)
    ...
    gmba_mask = (
        ...
        .clip(processing_region).rename("gmba_selected").unmask(0)
    )
    grid_screen = (
        quarter_degree_screen(processing_region, args)
        if args.apply_quarter_degree_screen
        else ee.Image.constant(1).clip(processing_region)
    )
    domain = gmba_mask.And(grid_screen).selfMask().rename("analysis_domain")
...
def build_shard_export_bundle(
    ...
    processing_region = batch_region.buffer(
        args.context_buffer_m, args.geometry_max_error_m
    )
    selected_shard = (
        selected_all.filter(ee.Filter.eq("region_id", region_id))
        .filterBounds(processing_region)
    )
    common = build_common(args, batch_region, processing_region, selected_shard)
    ...
    products = build_products(
        args, common, processing_region, ee.Number(otsu.get("threshold_c")),
    ...
    images = {
        ...
        "qa30m": ee.Image(products["qa30m"]).updateMask(otsu_valid_mask).clip(batch_region).toInt16(),
    }
    metadata = {
        "region_id": region_id,
        "shard_id": shard_id,
...
def make_asset_export_task(
    ...
    return ee.batch.Export.image.toAsset(
        ...
        pyramidingPolicy={".default": str(record["pyramiding_policy"])},
...
def run_formal_shard_preflight(
    ...
    if args.check_shard_id:
        candidates = [shard for shard in shard_plan if shard["shard_id"] == args.check_shard_id]
        if not candidates:
            raise ValueError(f"check shard is absent from resolved plan: {args.check_shard_id}")
        shard = candidates[0]
    else:
        shard = next(
            (
                candidate for candidate in shard_plan
                if str(candidate["region_id"]).startswith("R3_")
                and int(candidate["tile_x"]) == 5
                and int(candidate["tile_y"]) == 23
            ),
            min(shard_plan, key=lambda candidate: int(candidate["quarter_cell_count"])),
        )
    bundle = build_shard_export_bundle(args, selected_all, shard)
    shard_records = [
    ...
    online = {
        ...
        "otsu": ee.Dictionary(bundle["otsu"]).getInfo(),
        "treeline30m_bands": ee.Image(images["treeline30m"]).bandNames().getInfo(),
        "treeline1km_bands": ee.Image(images["treeline1km"]).bandNames().getInfo(),
        "qa30m_bands": ee.Image(images["qa30m"]).bandNames().getInfo(),
        "analysis_domain_count_1km": ee.Image(bundle["common"]["domain"]).reduceRegion(
...
def run_check(args: argparse.Namespace) -> Dict[str, object]:
    ...
    processing_region = batch_region.buffer(args.context_buffer_m, args.geometry_max_error_m)
    ...
    common = build_common(args, batch_region, processing_region, selected_check)
    ...
    formal_preflight = run_formal_shard_preflight(
        args, selected_all, shard_plan, export_records
    )
...
def start_exports(args: argparse.Namespace) -> Path:
    ...
    payload = {
        ...
        "otsu": "dynamic per shard on unchanged integer BIO1; threshold converted to Celsius",
        "overwrite_assets": args.overwrite_assets,
        "resume": args.resume,
        "phase": "PREFLIGHT",
        "tasks": records,
