...
## 2026-09-02 Step 2A recovery acceptance and offset-200 batch

- The hourly recovery monitor reached 80 `COMPLETED` tasks at `2026-09-01T18:09:20Z`, with zero failed, cancelled, or missing task IDs. All 80 destination Assets were present, non-empty `IMAGE`s: 40 `treeline30m` and 40 `qa30m`. Every Asset matched its registry mountain ID, run label `gmba_sayre_step2_v2_ts8_recovery_20260831`, configuration hash `d49bd16592abfc9bc5654068a49d5da8e522f7718afcfc49ba02c33fa416134f`, workflow, exact product band order, and the aligned 0.00025-degree EPSG:4326 grid. Git provenance split exactly as expected: ten first-recovery Assets at `a243efbcf80657ee5bd6a8bc844b678ebc1c88a8` and 70 remaining-recovery Assets at `89b06d7e86d860cf443ca8896931ff7f3e79f9e5`.
- Before the next batch, duplicate protection found no local registry, matching Earth Engine task, or matching output Asset for run label `gmba_sayre_step2_v2_batch200_o200_20260901`; the live queue had zero `READY` and zero `RUNNING` tasks. The trusted-receipt dry-run for `mountain-offset=200`, `max-mountains=200` passed with 400 expected tasks, products `treeline30m,qa30m`, validation ID `upstream-gmba-sayre-step1-20260831-v2`, and the same configuration hash.
- The offset-200 batch was submitted with queue safety limit 500. Registry `D:\\实验复现\\Globaltreeline_artifacts\\20260901-step2a-v2-batch200-o200\\tasks\\20260901T181357Z-treeline_gmba_sayre.json` contains 200 unique mountains, 400 unique task IDs, 400 unique destinations, 200 tasks per product, and zero mountain overlap with the offset-0 batch. The first post-submit snapshot at `2026-09-01T18:26:00Z` found all 400 task IDs, with one `RUNNING`, 399 `READY`, and zero failures, cancellations, missing IDs, or description mismatches. No Step 2B task was started.
